...
while (sum < 30){
  new <- sample(1:10, 1)
  ran_int <- c(ran_int, new)
  sum <- sum + new
  print(new)
}

print(paste0("You generated ", length(ran_int), " numbers that sum to ", sum, "."))
...
bmi_calc <- function(height, weight){
  bmi <- as.numeric(703*(weight/(height**2)))
  category <- ""
  if (bmi<18.5){
...
result <- bmi_calc(62, 102)
bmi_value <- as.numeric(result[1])
bmi_rounded <- round(bmi_value, 1)
health_category <- result[2]

print(paste("Your BMI is", health_category, "with a value of", bmi_rounded))
```
...
```{r}
mat <- matrix(c(vec1, vec2, vec3), ncol=3, byrow=FALSE)
colnames(mat) <- c("vec1", "vec2", "vec3")
mat
```
...
```{r}
sum_results <- lapply(lapply(lst, sum), function(x) x%%2==0)
print(sum_results)
```
...
```{r}
print(fruit_info$fruit[(order(fruit_info$weight))[length(fruit_info$weight)]])
```
...
for (i in 1:length(fruit_info$fruit)){
  if (fruit_info$weight[i] > 100){
    print(paste("The", fruit_info$fruit[i], "is heavy."))
  }else{
    print(paste("The", fruit_info$fruit[i], "is light."))
  }
}
```
